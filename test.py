...
if __name__ == "__main__":
    ...
    input_path = "large-101.in"
    G = read_input_file(f"{input_dir}/{input_path}")
    graph_name = input_path.split(".")[0]
    T1 = read_output_file(f"{output_dir}/{graph_name}.out", G)
    print('T1', average_pairwise_distance(T1))
    T2 = read_output_file(f"{new_output_dir}/{graph_name}.out", G)
    print('T2', average_pairwise_distance(T2))
